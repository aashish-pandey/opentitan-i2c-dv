...
class i2c_host_smoke_vseq extends uvm_sequence #(i2c_seq_item);
    ...
    task body();
        ...
        ral.target_fifo_status.read(status, fifo_rd, UVM_FRONTDOOR);
        `uvm_info("VSEQ", $sformatf("PRE-TX  target_fifo_status raw=0x%0h  acqlvl=%0d  txlvl=%0d",
                  fifo_rd,
        ...
        repeat(5) begin
            ...
            // Read ACQ FIFO level after each transaction
            // If FIFO is the cause of NACKs, acqlvl should grow each time
            ral.target_fifo_status.read(status, fifo_rd, UVM_FRONTDOOR);
            `uvm_info("VSEQ", $sformatf("POST-TX target_fifo_status raw=0x%0h  acqlvl=%0d  txlvl=%0d  (status=%s)",
                      fifo_rd,
                      ral.target_fifo_status.acqlvl.get_mirrored_value(),
